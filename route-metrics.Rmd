...
We can add a grey bar with low alpha transpacrency to slightly mask the less sever corners:
...
The `pracma::circlefit()` function will attempt to calculate the center and radius of a circle from a set of locations peovided as x and y coordinate lists. We can use the radius to determine conner severity numbers that are typically used in rally pace notes.
...
```{r}
# We may end up cutting corners?
tmp = st_segmentize(utm_routes[1,], units::set_units(1, m))
tmp2 = st_coordinates(tmp)
nrow(tmp2)
#22095 - 5m
# 23110 10m
# 21277 - 1m
...
route_convexity$radius = curvatures(route_convexity$Midpoint_X, 
                                    lead(route_convexity$Midpoint_X, 1),
                                    lead(route_convexity$Midpoint_X,2),
                                    route_convexity$Midpoint_Y,
                                    lead(route_convexity$Midpoint_Y),
                                    lead(route_convexity$Midpoint_Y,2)
                                    )
```
