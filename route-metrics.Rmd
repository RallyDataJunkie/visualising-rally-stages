...
### Convexity Profile of a State Route

We can profile the convextity of a stage roure in the terms of the convexity against the distance along the route in terms of a simple line chart. Recall that the sign of the convexity value denotes the direction of a corner (negative left, positive right).

Adding a grey zone to slightly mask the 
...
To try to get a feel for how the convexity changes alobg a route, we can render a 3D plot of the of the convexity using the `rayhader::ggplot()` function.
...
rgl::clear3d()
plot_gg(convexity_gg, scale=1000)
