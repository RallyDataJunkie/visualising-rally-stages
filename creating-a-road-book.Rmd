...
It might also be worth considering what we can learn from the [*FIA Rally Safety Guidelines 2020*](https://www.fia.com/sites/default/files/2printable_rally_safety_guidelines_1.pdf) [h/t `WRCStan`] about stage design and selection when evaluating rally stage routes metrics as well as road networks.
...
The `dodgr` package provides a range of tricks for modifying travel times on normal road netwroks, e.g. to account for time crossing roundabouts (for example, [Street networks and time-based routing
](https://atfutures.github.io/dodgr/articles/times.html)). It might be possible to add dummy nodes to a route prior to tight corners, for example, to add time weights to a route (or can `dodgr` route times take into account twistiness, elevation change etc?)

It might be quite amusing to try to define weight profiles for *rally_car* or rally cars under different weather and/or tyre conditions, perhaps based on models created from datasets of previous rally stage times or even car telemetry? A weighting profile determines the weighting applied to different road types. The default weighting profiles are stored in the `dodgr::weighting_profiles` list.
